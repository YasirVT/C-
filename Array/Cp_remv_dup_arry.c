...
int main(){
	...
	int arr[] = {1,4,2,1,4,1,31,3,42,1,5,21,21,21,21,12,12,12,12,21};
	int ar_i, ar_j,size,*ptr = NULL,count = 0,*ptr_rv = NULL;
	ptr_rv = ptr;
	...
	for(ar_i = 0; ar_i < size ; ar_i++){
		for(ar_j = ar_i + 1 ; ar_j < (size/2) + 1; ar_j++){
			...
			if(arr[ar_i] == arr[ar_j]){
				remove_dup(&arr[ar_j],(size - count) ); --ptr ; size--;
			}
			if(*ptr == arr[ar_i] || *ptr == arr[ar_j]){
				remove_dup(ptr, count - 1); size--; --ptr; 
			}
			if(*ptr == *(--ptr)){
